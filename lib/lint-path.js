...
var call         = Function.prototype.call
  ...
  , odiff        = require('es5-ext/lib/Object/diff')
...
lintDir = function (path, coreOptions, cb) {
	...
	deferred(
		...
		coreOptions.nocache ? {} : readFile(cachePath, 'utf-8')(
			function (content) {
				var data = parse(content)
				  , optdiff = odiff(data[optToken] || {}, coreOptions);
			...
			return (options[subpath] = readFile(confPath, 'utf-8')(
				function (content) {
					...
					return lstat(confPath)(function (stat) {
						...
						if ((cache[fpath] = stat.mtime.getTime()) != fcache[fpath]) {
				...
				return deferred((dir === subpath) ? coreOptions : options[dir])(
					function (pre) {
						if (conf) {
							if (conf['predef+']) {
								conf.predef = pre.predef ?
										pre.predef.concat(conf['predef+']).sort() : conf['predef+'];
								delete conf['predef+'];
							} else if (conf['predef-']) {
								if (pre.predef) {
						...
						return create((conf && conf['!root']) ? coreOptions : pre, conf);
					}
				);
			}));
		})(function () {
			options = omap(options, invoke('valueOf'));
					...
					return fcache[file] ? lstat(path + '/' + file)(
						function (stat) {
							return (((cache[file] = stat.mtime.getTime())
								== fcache[file]) && !toClear[dirname(file)]) ? null : file;
						}
					) : file;
				}).invoke(compact);
		});
		...
		if (!data.errors.length) {
			return (coreOptions.nocache || cache[data.filename]) ? null :
					lstat(path + '/' + data.filename)(function (stat) {
