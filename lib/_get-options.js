...
parse = function (data, path) {
	...
	opts = current = defineProperty({}, '_paths', d('', {}));
	if (!data) {
		return opts;
	}
	...
	if (data[0] === '@root') {
		defineProperty(opts, '_root', d('', true));
		data.shift();
	}
...
getDirOpts = memoize.call(function (path) {
	return readFile(resolve(path, '.lint'))(null, '')(function (content) {
		...
		return (data._root || (dirname(path) === path)) ? normalize(data) :
			getDirOpts(dirname(path))(function (parent) {
				if (parent) {
					defineProperty(data, '_parent', d('', parent));
...
getFileOpts = memoize.call(function (path) {
